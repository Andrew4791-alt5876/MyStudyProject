...
def card_number_generator(start_number: int, finish_number: int) -> Generator[str, Any, None]:
    ...
    try:
        if isinstance(start_number, int) and isinstance(finish_number, int) and start_number >= 0 and finish_number >= 0:
            for number in range(start_number, finish_number + 1):
                ...
                card_number_exit = (f"{card_number_str[:4]} {card_number_str[4:8]} "
                                f"{card_number_str[8:12]} {card_number_str[12:]}")
